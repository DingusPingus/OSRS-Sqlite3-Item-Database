...
def insert():
    ...
    for item in items:
        ...
        c.execute('''INSERT INTO ItemProperties(itemid, name, last_updated, incomplete, members, tradeable, tradeable_on_ge, 
        ...
        if(item.equipable_by_player):
            c.execute('''INSERT INTO ItemEquipment(id, attack_stab, attack_slash, attack_crush, attack_magic, attack_range, defence_stab,
                        ...
                        item.equipment.attack_ranged, item.equipment.attack_stab, item.equipment.defence_slash, item.equipment.defence_crush, item.equipment.defence_magic,
            ...
            if(item.equipable_weapon):
                c.execute('''INSERT INTO ItemWeapon(attack_speed, weapon_type) VALUES(?,?)''',(item.weapon.attack_speed, item.weapon.weapon_type))
    db.commit()
    db.close()